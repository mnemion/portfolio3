        ...
        <tbody>
            ...
            %>
                    <tr class="master_list">
                        ...
                        <td class="listcenter">
						    <select class="adlistsel3" id="status-<%= admin.getId() %>">
						        <option <%= admin.getStatus().equals("근무중") ? "selected" : "" %>>근무중</option>
						        <option <%= admin.getStatus().equals("퇴직중") ? "selected" : "" %>>퇴직중</option>
						    </select>
						</td>
                        <td class="listcenter"><button type="button" class="listclick" onclick="updateStatus(<%= admin.getId() %>)">적용</button></td>
                    </tr>
            <%
                }
            }
...
<div class="propagebt">
	<ul>
		<li><a href="<%="/config_main.do?page=" + (currentPage > 1 ? currentPage - 1 : 1) %>" <%= currentPage == 1 ? "style='color: #ccc;'" : "" %> > < </a></li>
		<% for(int i = 1; i <= totalPages; i++) { %>
		<li><a href="/config_main.do?page=<%= i %>"><%= i %></a></li>
		<% } %>
		<li><a href="<%="/config_main.do?page=" + (currentPage < totalPages ? currentPage + 1 : totalPages) %>" <%= currentPage == totalPages ? "style='color: #ccc;'" : "" %> > > </a></li>
	</ul>
</div>
</div>
</form>
